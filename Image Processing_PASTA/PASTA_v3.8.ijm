...
/* PArticle Shapes and Textures Analyzer (PASTA) v3.8 (January 24, 2023)
...
print("PASTA v3.8"); // Version label
	...
	for (i = 0; i < Imagefiles.length; i++) {
		...
		run("Duplicate...", "title=duplicate"+suffix); //duplicate the whole image to execute the process
			...
			setThreshold(0, 0);	
					 ...
					 */
					run("Analyze Particles...", "size=rbut1n-Infinity pixel show=Outlines exclude add"); // Create multiparticle outlines
...
//--- STEP 2 - IMAGE PROCESSING	
	...
	run("Point Tool...", "type=Hybrid color=Yellow size=Medium auto-measure");
	setTool("point");
		//--- Picking RGB intensities for the background (outside of particle area)
		checkRGB2 = "No";
		...
		print(""); // Create a space line in the Log between the two steps
			for (i10 = 0; i10 < Psingles.length; i10++) {
				...
				//-- Open the file
					...
					run("Select None");
					orgb=getTitle();					
					oRGbWidth = getWidth(); // Get Width of the opened RGB single file for selection check further below; the width refers to the whole image, not the particle
					itype = bitDepth(); // get the type format of the image: 24 = RGB color; 8 = 8-bit
					SummaLab = Psingles[0];
				...
				//--- Create FORM and selection
					...
					//--- Start Thresholding with check if image is in a RGB or greyscale format
						...
						WidthCheck = List.getValue("Width"); // Get the width of whatever selection exists. If the selection is on the area outside the particle, the width will be that of the whole single image frame, not the particle area
	...
	print("Processing Crystallinity - Vesicularity");
			...
			while (sDirL < pickL) { // This loop allows script to exit once all segmented files have been created
				...
				for (isg = 0; isg < pickList.length; isg++) {
					...
					setBatchMode("hide"); // All the current images and related processing is hidden, making processing much faster ; MUST BE ON "hide" once the code has been fixed
					...
					open(myDir1+File.separator+pickList[isg]);
							...
							run("Select None");						
							selframeWidth = getWidth(); // Get width of the single image for segmentation. The width refers to the whole image, not the particle area
						...
						// --- Start feature Extraction  
							...
							if (chbVES==true) {
								...
								run("Analyze Particles...", "size=&VESpse-Infinity pixel circularity=0.00-1.00 show=Masks clear");															
								run("Invert LUT");															
								run("Create Selection");	
							...
							if (chbOXX==true) {
								...
								run("Analyze Particles...", "size=&OXXpse-Infinity pixel circularity=0.09-1.00 show=Masks clear"); // min value for circularity as oxides are usually equant. Used to filter noise 
								run("Invert LUT");
								run("Create Selection");
							...
							if (grXLS1 == true) {
								...
								//--- End of Colour Thresholding-------------							
								run("Analyze Particles...", "size=&XLS1pse-Infinity pixel show=Masks clear");
								run("Invert LUT");
								run("Create Selection");
								close(dupXLS1); // close duplicate for Grey Crystal 1
								rename(dupXLS1);	
							} //--- End Grey crystals 1 extraction 
							//waitForUser("XLS1 file");	
							...
							if (grXLS2==true) {
								...
								run("Analyze Particles...", "size=&XLS2pse-Infinity pixel show=Masks clear");
								run("Invert LUT");
								run("Create Selection");			
								close(dupXLS2); // close duplicate for Grey Crystals 2
								rename(dupXLS2);
							} //--- End Grey Crystals 2 extraction
							//waitForUser("XLS2 file");	
							...
							if (grXLS3==true) {
								...
								run("Analyze Particles...", "size=&XLS3pse-Infinity pixel show=Masks clear");
								run("Invert LUT");
								run("Create Selection");
						...
						//---Set base image with Groundmass
							selectWindow(pickList[isg]);
							//waitForUser("sel base file");	// Check for debugging							
							...
							//--- End of Colour Thresholding-------------
							//waitForUser("see thresolded base file");	// check for debugging

							// The following commands do the filling of the base image with the groundmass and outside area with the values defined at the beginning of the code
							setThreshold(0, 0);		// it select the particle area in black		
							run("Create Selection"); // Create a selection on the particle area
							List.setMeasurements;	
							WidthCheckSegBase = List.getValue("Width"); // Get the width of the particle area
							...
							setForegroundColor(gfo, gfo, gfo); // Fill the groundmass of the particle				 
							run("Fill");
							...
							run("Fill"); // 									
							run("Make Inverse"); // Now select the area outside the particle
							setForegroundColor(bkg, bkg, bkg); // and fill with a white color
							run("Fill");							
							run("Select None");
							rename("segbase");
							segbase = getTitle();
						...
						if (chbVES==true) {
							selectWindow(dupVES);
							List.setMeasurements;		
							WidthCheckVesSel = List.getValue("Width"); // Get the width of the vesicle selection
							if (WidthCheckVesSel >= selframeWidth) { // Check if the selection is actually on the feature, and if no reverse the selction
								//print("Selection on outside area. Inverting back on feature selection...");
								run("Make Inverse");							
							}
						...
						//showMessageWithCancel("", "Check XLS 2 result");
						...
						//showMessageWithCancel("", "Check XLS 3 result");	
